...
def make_CDNN_classifier(input_tuple, conv_layers, dense_layers, n_classes, dropout_rate=0.1,loss='categorical_crossentropy', optimizer='adam', metrics=['accuracy'], n_conv=3, channel_order='channels_last'):
    ...
    for _ in range(0, conv_layers[1]):
        c = Conv2D(filters=conv_layers[0],
                   ...
                   strides=(1, 1),
                   padding='same',
                   data_format=channel_order)(b)
        b = BatchNormalization()(c)
        added = kadd([added, b])
    mp = MaxPooling2D((2, 2))(added)
    flatout = Flatten()(mp)
    output = Dense(int(n_classes), activation='softmax')(flatout)
    Omodel = Model(inputs=inputs, outputs=output)
...
def main(testing=False):
    ...
    if FramelevelORword:
        ...
        cdnn_dict_name = 'rescdnn_gridsearch_records_ALL.pk'
        if not TrainAll:
            ByCount = 8000
            ...
            cdnn_dict_name = f'rescddnn_gridsearch_records_{ByCount}.pk'
    else:
